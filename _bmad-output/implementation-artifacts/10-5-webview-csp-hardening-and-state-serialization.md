...
## Defect-fix / reconciliation — `script-src 'wasm-unsafe-eval'` (Epic 10 manual smoke #2, 2026-06-03)

**Defect (HIGH, lead's manual interactive smoke, Rule 14, real VS Code Extension Development Host):** after defect #1 (room not opening) was fixed, a room now opens but **every message BODY renders empty** — only the `@handle` + the 👍 reaction chip show; the markdown body text is blank for ALL posts (even plain-text bodies).

**Root cause (traced + verified in code):** the byte-shared `@agentbbs/ui-shared` `MarkdownView` renders markdown via `renderMarkdown` (`packages/ui-shared/src/markdown/render-markdown.ts:176`), which `await prewarmHighlighter()` **before producing ANY html**. `prewarmHighlighter` → Shiki `createHighlighter` (`packages/ui-shared/src/markdown/highlight.ts:82`), whose oniguruma regex engine is **WebAssembly compiled in the webview** (the wasm is INLINED in `dist/webview/main.js` — confirmed 4 wasm markers present; it is a WASM *compile*, NOT a network fetch). The Story-10.5 strict CSP set `script-src 'nonce-<n>'` with **no wasm token**, so Chromium (the Electron webview) blocked `WebAssembly.instantiate` → `createHighlighter` never resolved → `renderMarkdown` never resolved → `MarkdownView`'s body `useState('')` stayed empty for EVERY post (the empty seed because `isHighlighterWarm()` is also forever `false`). happy-dom does not enforce CSP, so only the real-webview smoke surfaced it (Rule 12).

**Fix (CLIENT/host layer only — Rule 13):** added `'wasm-unsafe-eval'` to the ROOM shell `script-src` in `apps/vscode-extension/src/webview/webview-html.ts` → `script-src 'nonce-<n>' 'wasm-unsafe-eval'`. The **compose** shell was audited and deliberately left **wasm-free**: none of its 4 surfaces (create-project / post-announcement / join-project / focus) mounts `MarkdownView`/Shiki — they render plain form inputs only — so no WASM compile ever runs there; carrying the token would needlessly loosen the tighter floor. The room + compose CSPs are separate arrays in the same builder file; only the room array changed. ALL other directives are byte-unchanged: `default-src 'none'`, no `'unsafe-inline'`/`'unsafe-eval'`, own-origin `img-src`/`font-src`, `style-src` nonce+cspSource, `connect-src 'none'`, nonce'd scripts. `git diff HEAD -- packages/core packages/mcp-server packages/ui-shared` stays **EMPTY** — the fix is the host's CSP string, NOT the renderer.

**Rule 8 reconciliation (explicit, in writing):** Story 10.5 AC1 said the strict CSP carries "no `unsafe-inline`/`unsafe-eval`". `'wasm-unsafe-eval'` is a **distinct, narrower CSP token** — it is NEITHER `'unsafe-inline'` NOR `'unsafe-eval'`. It permits **WebAssembly compilation/instantiation ONLY**; it opens **no** JavaScript `eval`/`new Function`/inline-script surface. Therefore the strict Story-10.5 posture **stands unchanged**: no `unsafe-inline`, no `unsafe-eval`, `default-src 'none'`, nonce'd scripts, `connect-src 'none'`, own-origin img/font. `'wasm-unsafe-eval'` is permitted **only** because the shipped byte-shared ui-shared inert-markdown renderer needs WASM, and it does **not** weaken the inert/no-script (NFR12) guarantee — the markdown stack is still markdown-it `html:false` → DOMPurify → Shiki class-spans, with zero active content injected. This is a faithful narrowing-aware extension of AC1, not a contradiction of it.

**Tests (Rule 7/8/10 content-guard PIN):** updated the CSP content-guards so the required token is now PINNED (its absence — the very regression that broke rendering — turns them RED), while still asserting the two FORBIDDEN tokens are absent. Care taken: `'wasm-unsafe-eval'` legitimately contains the substring `unsafe-eval`, so the guards assert the **exact quoted** `'unsafe-inline'`/`'unsafe-eval'` tokens (plus a `/(?<!wasm-)unsafe-eval/` negative-lookbehind) rather than a bare substring, which would false-fail on the wasm token.
- `webview-html.csp.test.ts`: `assertStrictCsp` now requires `script-src 'nonce-<n>' 'wasm-unsafe-eval'`; added a mutation loosening "script-src missing wasm-unsafe-eval" (the new PIN).
- `webview-html.qa.test.ts`: room `script-src` exact-match + whole-CSP floor updated to the precise-token form.
- `room-panel.test.ts`: room nonce-CSP guard updated to the precise-token form.
- `compose-panel.test.ts`: UNCHANGED — its `not.toContain('unsafe-eval')` stays correct because the compose shell remains wasm-free (corroborates the deliberate room/compose divergence).

**Rule 7 mutation (both non-vacuous, production reverted byte-identical, gates GREEN after):** (a) dropped `'wasm-unsafe-eval'` from the REAL room CSP → 4 guards RED across all three files (the PIN works); (b) added `'unsafe-inline'` to the REAL room CSP alongside the wasm token → 5 guards RED (the precise-token check is NOT fooled by the wasm token's presence). Reverted byte-identically both times; `git diff HEAD -- packages/core packages/mcp-server packages/ui-shared` EMPTY.

**Rule 12 (runtime is the gate):** the *runtime* "WASM compiles → bodies actually render" behavior is NOT verifiable under happy-dom (no CSP enforcement). The content-guard PINS the CSP string; the lead's real Ext-Dev-Host re-smoke is the load-bearing runtime evidence. Rebuilt the webview bundle (`pnpm --filter @agentbbs/vscode-extension run build:bundle`) so the lead's re-smoke loads the fixed CSP. Left UNCOMMITTED for the lead to re-smoke + commit.

### File List (defect-fix)

Modified:
- apps/vscode-extension/src/webview/webview-html.ts (room `script-src` += `'wasm-unsafe-eval'`; JSDoc/comments explain WHY + the deliberate compose wasm-free divergence)
- apps/vscode-extension/src/webview/webview-html.csp.test.ts (PIN the wasm token; precise forbidden-token asserts; new mutation loosening)
- apps/vscode-extension/src/webview/webview-html.qa.test.ts (room script-src + whole-CSP floor → precise-token form)
- apps/vscode-extension/src/room-panel.test.ts (room nonce-CSP guard → precise-token form)

### Change Log (defect-fix)

- 2026-06-03 — Epic 10 manual smoke #2 defect-fix: empty message bodies in the VS Code room webview. Added `'wasm-unsafe-eval'` to the room webview `script-src` (the byte-shared ui-shared Shiki/oniguruma WASM markdown renderer was CSP-blocked → `renderMarkdown` never resolved). Compose CSP left wasm-free (no markdown there). Rule 8 reconciliation recorded above (the strict no-unsafe-inline/no-unsafe-eval posture stands; `wasm-unsafe-eval` is a distinct narrow token). CSP content-guards updated to PIN the token (mutation-tested RED on its removal). Rule 13 clean. Bundle rebuilt. Left uncommitted for the lead's real-host re-smoke + commit.
