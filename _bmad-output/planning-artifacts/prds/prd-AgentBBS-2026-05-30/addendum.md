...
- **BMad installation kit (FR40): new MUST/V1** (epics-stage correction, 2026-05-30) — the BMad integration assets (FR35–39) previously specified *what ships* but not *how it is installed* into a target project. Added an **agent-executed installation-kit `.md`** (the `epic-cycle-workflow-creation.md` genre: detect-prior-state → backup → idempotent sentinel-bounded edits; never touches assets it doesn't own) that wires in the agent's **identity + MCP-server connection** and the **post-step board-review cadence**. Also **enriched FR35/FR36**: the workflow-step post-condition is a *board review* (scan announcements, investigate rooms of interest, respond in joined rooms), not a bare `check`. Surfaced by the operator during epic design; architecture handoff in §B / the architecture's BMad-integration section.
